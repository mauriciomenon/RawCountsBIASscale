...
return_frame = tk.LabelFrame(app, text="Resultados")
return_frame.pack(fill="both", expand="yes")

# Ponteiro do amperimetro (valor calculado)

l_amp_text = tk.Label(return_frame, text="Valor (A)", anchor=tk.W)
l_amp_val = tk.Label(return_frame, text="", anchor=tk.W)
l_amp_text.grid(row=0, column=0, sticky=(tk.W, tk.E))
l_amp_val.grid(row=0, column=1, sticky=(tk.W, tk.E))
